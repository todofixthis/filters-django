---
name: release
description: Use when preparing or publishing a new release of phx-filters-django — covers release notes, version bump, build, PyPI upload, and GitHub release creation
---
# Release
...
```bash
shasum -a 256 dist/phx_filters_django-* >> release-<version>.md
```
...
```bash
for f in dist/phx_filters_django-*; do gpg --detach-sign --local-user "$GPG_KEY" "$f"; done
# Creates dist/phx_filters_django-*.sig alongside each artefact
```
...
gh release create <version> dist/* \
  --title "Django Filters v<version>" \
